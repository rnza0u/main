...
When using the [`run`](../cli/run) command, the `-s` option (`--selector` in its long form) allows you to use one of these named selectors. 

##### Default selector

When using the `run` command, you can omit the project selector.
