...
It can be installed on the following platforms :

- Linux
- Mac OS
- Windows

Pre-built binaries are provided for these three platforms.

You can find direct download links for each platform [here](/downloads).
...
These scripts might assume that you are using `sudo` on your system. You can also directly run them directly as `root`.
...
- <BrowserOnly>{() => <a target="_blank" href={downloadUrl('latest', 'x86_64-windows')}>Download the latest package for Windows</a>}</BrowserOnly>.
...
<BrowserOnly>{
  () => <WithLatestVersion>{
      version => <>
        <p>
          You can build your own binary <a target="_blank" href={`https://github.com/Hakhenaton/main/tree/blaze-${version}`}>from the source code</a> using different methods.
        </p>
        
        <p>You will need a Rust nightly toolchain (&gt;= 1.81.0). You can download it using <a href="https://rustup.rs"><code>rustup</code></a> :</p>
        ...
        <CodeBlock language='sh'> 
          {`cargo +nightly install -Z bindeps blaze-cli@${version}`}
        </CodeBlock>
        ...
        <CodeBlock language='sh'> 
          {
            [
              `git clone --depth 1 --branch blaze-${version} https://github.com/Hakhenaton/main.git`,
...
The generated `workspace.json` is your monorepo main configuration file.

You can now start adding your projects.
...
First, explore the `workspace.json` file. There should be a project reference named `example-project`.
...
Any target in Blaze rely on what we call an *executor*.
